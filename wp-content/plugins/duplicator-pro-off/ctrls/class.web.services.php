...
class DUP_PRO_Web_Services extends AbstractAjaxService
{
    public function init()
    {
        ...
        $this->addAjaxCall('wp_ajax_nopriv_duplicator_pro_try_to_lock_test_sql', 'try_to_lock_test_file');
        ...
        /* Tests */
        $this->addAjaxCall('wp_ajax_duplicator_pro_build_package_test', 'duplicator_pro_build_package_test');
    ...
    function duplicator_pro_restore_backup_prepare_callback()
    {
        ...
        $result = DUPLICATOR_PRO_SSDIR_URL . '/' . $package->Installer->getFileName() . '?dup_folder=dupinst_' . $package->Hash;
    ...
    {
        AjaxWrapper::json(array(__CLASS__, 'duplicator_pro_restore_backup_prepare_callback'), 'duplicator_pro_restore_backup_prepare', $_POST['nonce'], 'export');
    }
    ...
    function duplicator_pro_ftp_send_file_test()
    {
        ...
        try {
            ...
            if ($dest_filepath === false) {
                throw new Exception(__("Couldn't create the destination temp file for the FTP send test.", 'duplicator-pro'));
            }
            ...
            if ($ftp_client->delete($remote_source_filepath) == false) {
                $statusMsgsObj->addMessage(__('Couldn\'t delete the remote file', 'duplicator-pro'));
    ...
    function duplicator_pro_sftp_send_file_test()
    {
        ...
        try {
            ...
            $sFtpAdapter = new DUP_PRO_PHPSECLIB();
            $sftp          = $sFtpAdapter->connect($server, $port, $username, $password, $private_key, $private_key_password, $statusMsgsObj);
            ...
            if (!$sftp->file_exists($storage_folder)) {
                ...
                $sFtpAdapter->mkDirRecursive($storage_folder, $sftp);
                if (!$sftp->file_exists($storage_folder)) {
            ...
            if ($sftp->put($storage_folder . $basename, $source_filepath, $sFtpAdapter->sourceLocalFiles | $sFtpAdapter->sFtpResume)) {
                ...
                $statusMsgsObj->addMessage(__('Error uploading test file, maybe the directory does not exist or you have no write permissions', 'duplicator-pro'));
    ...
    function duplicator_pro_local_storage_test()
    {
        DUP_PRO_Handler::init_error_handler();
