...
<table class="dpro-edit-toolbar">
    <tr>
        ...
        <td>
            <?php include(DUPLICATOR____PATH . '/views/packages/main/create.package.nav.buttons.php'); ?>
        </td>
    </tr>
</table>
...
<?php
$messageText = '';
$messageText .= '<br><b>' . DUP_PRO_U::__("Please Retry:") . '</b><br/>';
$messageText .= DUP_PRO_U::__("Unable to perform a full scan and read JSON file, please try the following actions.") . '<br/>';
$messageText .= DUP_PRO_U::__("1. Go back and create a root path directory filter to validate the site is scan-able.") . '<br/>';
$messageText .= DUP_PRO_U::__("2. Continue to add/remove filters to isolate which path is causing issues.") . '<br/>';
$messageText .= DUP_PRO_U::__("3. This message will go away once the correct filters are applied.") . '<br/><br/>';

$messageText .= '<b>' . DUP_PRO_U::__("Common Issues:") . '</b><br/>';
$messageText .= DUP_PRO_U::__("- On some budget hosts scanning over 30k files can lead to timeout/gateway issues. Consider scanning only your main WordPress site and avoid trying to backup other external directories.") . '<br/>';
$messageText .= DUP_PRO_U::__("- Symbolic link recursion can cause timeouts.  Ask your server admin if any are present in the scan path.  If they are add the full path as a filter and try running the scan again.") . '<br/><br/>';

$messageText .= '<b>' . DUP_PRO_U::__("Details:") . '</b><br/>';
$messageText .= DUP_PRO_U::__("JSON Service:") . ' /wp-admin/admin-ajax.php?action=duplicator_pro_package_scan<br/>';
$messageText .= DUP_PRO_U::__("Scan Path:") . '[' . duplicator_pro_get_home_path() . ']<br/><br/>';
?>
                            message += <?php echo SnapJson::jsonEncode($messageText); ?>;
