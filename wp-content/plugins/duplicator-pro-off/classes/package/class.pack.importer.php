...
class DUP_PRO_Package_Importer
{
    ...
    public function prepareToInstall()
    {
        ...
        if ($this->determineInstallerFolder() == 'none') {
    ...
    protected function determineInstallerFolder()
    {
        if (empty($this->insPathType)) {
            $this->insPathType = ($this->isLite() ? 'home' : 'duplicator');
            if ($this->insPathType == 'duplicator') {
            ...
            /**
             * Home is always considered a valid path because in some cases (hsoting with slow hdd and some cache systems) 
             * the check returns a false negative
             */
            $this->insPathType = 'home';
            /*
    ...
    {
        switch ($this->determineInstallerFolder()) {
            case 'duplicator':
    ...
    protected static function getFoldersToCheck()
    {
        ...
        $customPath = DUP_PRO_Global_Entity::get_instance()->import_custom_path;
