...
require_once(DUPLICATOR____PATH . '/classes/package/class.pack.database.php');
require_once(DUPLICATOR____PATH . '/classes/package/class.pack.archive.file.list.php');
require_once(DUPLICATOR____PATH . '/classes/package/class.pack.importer.php');
require_once(DUPLICATOR____PATH . '/classes/package/class.pack.recover.php');
    ...
    public function getLocalPackageFilePath($file_type)
    {
        switch($file_type) {
            case DUP_PRO_Package_File_Type::Installer:
                ...
                $fileName = $this->Installer->getFileName(true);
                break;
...
// Indicates if all storages have finished (succeeded or failed all-together)
    ...
    private function delete_local_storage_files()
    {
        ...
        $installer_filename  = $this->Installer->getFileName(true);
    ...
    public static function deletePackageFilesInDir($nameHash, $dir, $deleteLogFiles = false)
    {
        ...
        foreach ($globFiles as $globFile) {
            if (!$deleteLogFiles && SnapString::endsWith('_log.txt', $globFile)) {
                continue;
            }
...
//START LOGGING
            ...
            if ($this->build_progress->initialized == false) {
                ...
                if(License::getType() != License::TYPE_BUSINESS_GOLD) {
...
//FINAL REPORT
    ...
    public function build_integrity_check()
    {
        ...
        //------------------------
        //INSTALLER CHECK:
        $exe_temp_path = SnapIO::safePath(DUPLICATOR_PRO_SSDIR_PATH_TMP . '/' . $this->Installer->getFileName(true));
    ...
    {
        try {
            if (($schedule = DUP_PRO_Schedule_Entity::get_by_id($this->schedule_id)) === null) {
            ...
            $data = array(
                ...
                'storageNames' => array_map(function ($s) {return $s->name; }, $this->get_storages()),
...
// Use the manual template for the data while the $template_id is used just to record where it originally came from
    ...
    public function update($die = true)
    {
        ...
        $wpdb->flush();
        if (
            $wpdb->update($wpdb->base_prefix . "duplicator_pro_packages", 
                array(
                    'status'  => $this->Status,
                    'package' => $packageObj
                ), 
    ...
    private function build_cleanup()
    {
        if (($files = SnapIO::regexGlob(DUPLICATOR_PRO_SSDIR_PATH_TMP, [])) !== false) {
            $filesToStore = array(
                $this->Installer->getFileName(true),
                $this->Archive->File,
            );
