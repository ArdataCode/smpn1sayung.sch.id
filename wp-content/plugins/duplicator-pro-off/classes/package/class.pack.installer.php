...
class DUP_PRO_Installer extends AbstractJsonSerializable
{
    ...
    {
        return SnapIO::safePath(DUPLICATOR_PRO_SSDIR_PATH . "/" . $this->getFileName(true));
    }

    /**
     * Get the installer file name
     *
     * @param bool $local if set to true returns the file name as saved on the disk
     * @return string
     */
    public function getFileName($local = false)
    {
        if (!$local) {
            return $this->File;
        }

        return apply_filters('duplicator_pro_installer_local_file_name', $this->File);
    }
    ...
    {
        $this->File = $nameHash . '_' . DUP_PRO_Global_Entity::get_instance()->installer_base_name;
    }
    ...
    private function create_enhanced_installer()
    {
        ...
        $installer_filepath     = SnapIO::safePath(DUPLICATOR_PRO_SSDIR_PATH_TMP) . "/" . $this->getFileName(true);
...
HEADER;
    ...
    private function getExtraFilesLists($checkExists = true)
    {
        ...
        $result[] = array(
            'sourcePath'  => SnapIO::safePath(DUPLICATOR_PRO_SSDIR_PATH_TMP) . "/" . $this->getFileName(true),
