...
/**
 * Entity layer for the package template
 *
 * Standard: Missing
 *
 * @package DUP_PRO
...
class DUP_PRO_Package_Template_Entity extends AbstractEntity implements UpdateFromInputInterface
{
    public $name                          = '';
    public $notes                         = '';
    //MULTISITE:Filter
     ...
     *
     * @param DUP_PRO_Package_Template_Entity $data input data
     *
     * @return self
     */
    public static function createFromImportData(DUP_PRO_Package_Template_Entity $data)
    {
        $instance = new self();
    ...
    public function getEditUrl()
    {
        ...
        return $baseUrl . '&' . http_build_query(array(
            'tab' => 'templates',
            'inner_page' => 'edit',
            'package_template_id' => $this->getId()
        ));
    }
    ...
    {
        if (($source_template = self::getById($source_template_id)) === false) {
            throw new Exception('Can\'t get tempalte id' . $source_template_id);
        }
        ...
        foreach ($props as $prop) {
            ...
            $prop->setValue($this, $prop->getValue($source_template));
        }
