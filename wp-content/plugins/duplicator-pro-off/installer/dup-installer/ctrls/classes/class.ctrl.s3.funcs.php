...
final class DUPX_S3_Funcs
{
    ...
    {
        return PrmMng::getInstance()->getValue(PrmMng::PARAM_REPLACE_MODE) === "mapping";
    }
    ...
    protected function wpConfigUpdate(WPConfigTransformer $confTransformer)
    {
        ...
        try {
            ...
            if (License::getType() >= License::TYPE_FREELANCER && $paramsManager->getValue(PrmMng::PARAM_GEN_WP_AUTH_KEY)) {
...
LONGMSG;
    ...
    protected function configurationUrlsAndPaths(WPConfigTransformer $confTransformer)
    {
        ...
        $pathPluginsDef = $urlContentDef . '/plugins';
        ...
        $pathMuPluginsDef = $urlContentDef . '/mu-plugins';
    ...
    protected function updateBlogName() {
        ...
        DUPX_DB::mysqli_query(
            $this->dbh, "UPDATE `" . $escapedOptionTable . 
        ...
        if (DUPX_InstallerState::isAddSiteOnMultisite()) {
            ...
            foreach ($overwriteMapping as $map) {
                ...
                $this->updateOptionsUrls(
                    DUPX_DB_Functions::getOptionsTableName($targetInfo['blog_prefix']),
                    $urlNew,
                    $siteUrl
                );
